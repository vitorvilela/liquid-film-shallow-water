...
const scalar slipFactor[] = 0.0001;
...
{ 
  ...
  foreach() {       
    ...
    if(r < wetAreaRadius) {     
      ...
      double localThicknessSource = (dropletVolume/avgDropletVolume)*((Delta*Delta)/initialWetArea)*filmThicknessRate;
      
      accumulatedThickness += localThicknessSource;
           
      hs[] = localThicknessSource; 
      
      foreach_dimension()
          hus.x[] = filmThicknessRate*u.x[];
...
{   
  ...
  foreach() {   
    ...
    if(r < wetAreaRadius) {
      ...
      // Fourier modes: sin(x * k * pi), where k is the wave number, must cover the range 0 < x < 1 
//       double vel = 0.5 * (10423481112.5*pow(r, 4.) + 15379411.8*pow(r, 3.) - 635221.8*pow(r, 2.) - 161.2*r + 20.5) * fabs(sin((t/sprayDuration) * 100 * M_PI));
      velocity.x[] = 0; //vel*sin(sprayAngle*M_PI/180); 
      velocity.y[] = 0.;
...
event output_gifs (i += 100; t<=totalSimulation) {
