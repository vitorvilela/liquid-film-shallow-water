...
// Injected Mass [kg]
#define injectedMass 10*15.6e-6
// Mass Rate [kg/s]
...
const scalar slipFactor[] = 0.0002;
...
{ 
  ...
  foreach() { 
    ...
    if(r < wetAreaRadius)   {
      ...
      velocity.x[] = 0.; // vel*sin(sprayAngle*M_PI/180); // + (1-fabs(y)/wetAreaRadius)*vel*cos(sprayAngle*M_PI/180); // vel*sin(sprayAngle*M_PI/180); // + 0.5*vel*cos(sprayAngle*M_PI/180)*(r/wetAreaRadius)*(x/fabs(x));
...
{   
  ...
  foreach() {   
    ...
    if(r < wetAreaRadius) {
      ...
      velocity.x[] = vel*sin(sprayAngle*M_PI/180); 
      velocity.y[] = 0.;
