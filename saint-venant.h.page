  ...
  foreach() {
	...
	foreach_dimension()
	  //uo.x[] = (hold*ui.x[] + dt*dhu.x[])/ho[];
          //vitor
          uo.x[] = (hold*ui.x[] + dt*(dhu.x[]+hus.x[]))/ho[];  
      }
...
double update_saint_venant (scalar * evolving, scalar * updates, double dtmax)
{
  ...
  *Fh* and *Fq* will contain the fluxes for $h$ and $h\mathbf{u}$
  ...
  for (int l = 0; l < nl; l++) {
    ...
    foreach() {
      double dhl = layer[l]*(Fh.x[1,0] - Fh.x[] + Fh.y[0,1] - Fh.y[])/(cm[]*Delta);
      dh[] = - dhl + (l > 0 ? dh[] : 0.);
      //vitor: mass conservation source term (here or directly in advance function)
      //dh[] = - dhl + (l > 0 ? dh[] : 0.) + layer[l]*hs[];
      foreach_dimension()
	dhu.x[] = (Fq.x.x[] + Fq.x.y[] - S.x[1,0] - Fq.x.y[0,1])/(cm[]*Delta);
        //vitor: momentum conservation source term (here or directly in advance function)
        //dhu.x[] = (Fq.x.x[] + Fq.x.y[] - S.x[1,0] - Fq.x.y[0,1])/(cm[]*Delta) + hus.x[];
